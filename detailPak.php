...
$query1 = "SELECT serial_number,restaurant.name as n1, city.name as n2, time as n3 FROM object_point JOIN restaurant ON restaurant.id = object_point.id_restaurant , city where st_contains(city.geom, restaurant.geom) and id_package='$id'  AND object_point.status = '3'";
$query2 = "SELECT serial_number,tourism.name as n1, city.name as n2, time as n3 FROM object_point JOIN tourism ON tourism.id = object_point.id_tourism, city where st_contains(city.geom, tourism.geom) and id_package='$id'  AND object_point.status = '4'";
$query3 = "SELECT serial_number,souvenir.name as n1, city.name as n2, time as n3 FROM object_point JOIN souvenir ON souvenir.id = object_point.id_souvenir, city where st_contains(city.geom, souvenir.geom) and id_package='$id'  AND object_point.status = '1'";
$query4 = "SELECT serial_number,workship_place.name as n1 , city.name as n2, time as n3 FROM object_point JOIN workship_place ON workship_place.id = object_point.id_workship_place , city where st_contains(city.geom, workship_place.geom) and id_package='$id'  AND object_point.status  = '2'";
