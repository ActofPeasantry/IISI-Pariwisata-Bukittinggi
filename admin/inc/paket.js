 var server = "http://localhost/ta_hausa/";
    var map;
    var markersDua = [];
    var markersDkt = [];
    var centerBaru;
...
function cariobjek(){
      ...
      if (objek!=0 && city!=0){
        $.ajax({url: "http://localhost/pariwisata/cariobjek.php?objek="+objek+"&city="+city, data: "", dataType: 'json', success: function(rows){
          console.log(rows);
          console.log("http://localhost/pariwisata/cariobjek.php?objek="+objek+"&city="+city); 
            if (objek=="1") {
              // console.log("jalan1");
              for (var i in rows){
...
function simpanpaket(){
  ...
  if (paket_id.length >0){
    ...
    if( name == '' || price == ''){
      ...
      $.ajax({url: "http://localhost/pariwisata/simpanpaket.php?id="+id+"&id_travel="+id_travel+"&name="+name+"&price="+price, data: "", dataType: 'json', success: function(rows){
          //console.log(rows);
          console.log("http://localhost/pariwisata/simpanpaket.php?id="+id+"&id_travel="+id_travel); 
